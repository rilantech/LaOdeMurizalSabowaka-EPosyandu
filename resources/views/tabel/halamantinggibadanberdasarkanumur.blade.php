                ...
                <!-- ============================================================== -->
                    ...
                    <div class="card-body">
                        ...
                        <div class="col-12 mb-4">
                            <h5 class="card-title text-center">Tinggi Badan Berdasarkan Umur</h5>  
                        </div> 
                            ...
                            <div class="form-group col-4" style="margin-top:-25px;">
                                <label for="agama_ibu">Pilih Data</label>
                                    <select class="form-select @error('agama_ibu') is-invalid @enderror"  aria-label="Default select example" name="agama_ibu" id="agama_ibu" onchange="pindahHalaman(this.value)">
                                          ...
                                          <option value= "/datakms" {{ old('agama_ibu') }}>Tinggi Badan Berdasarkan Umur</option>
                                    </select>
                                          @error('agama_ibu')
                                          <div class="invalid-feedback">
                                            {{ $message }}
                        ...
                        <div class="col-4">
                            <button type="button" class="btn btn-outline-dark rounded-pill fw-bold text-white" style="background-color: #4DA8DA;margin-left: 250px;" onclick="window.location='{{ url('/tambahtinggibadan') }}';" style="margin-left: 75px; background-color: #EE66A6;">Tambah</button>
                        </div>
                        </div>
                       ...
                       <div class="card-body">
                        <div class="table-responsive bg-white">
                            <table class="table table-bordered table-hover text-center">
                                ...
                                <tbody>
                                    @php
                                    $nomor = 1 + (( $datatinggibadan->currentPage()-1)* $datatinggibadan->perPage());;
                                @endphp
                                    @foreach ($datatinggibadan as $row)
                                    <tr>
                                        <td>{{ $nomor ++ }}</td>
                                        <td>{{ $row->nama_anak }}</td>
                                        <td>{{ $row->nama_ibu }}</td>
                                        ...
                                        <td>  <div class="row">
                                            <div class="col-6 text-center">
                                                <button onclick="window.location='{{ url('tinggibadan/'.$row->id) }}'" type="button" class="btn btn-success btn-md" title="Detail">
                                                     <i class="fas fa-list"></i>
                                                </button>
                                               </div>
                                            <div class="col-6 text-center">
                                             <form onsubmit="return deleteData('{{ $row->nama_anak }}')" action="{{ url('tinggibadan/'.$row->id) }}" method="POST"> 
                                              @csrf
                                              @method('DELETE')
                            ...
                            {!! $datatinggibadan->appends(Request::except('page'))->render() !!}
                        </div>
                    </div>
                      <script>
