                ...
                <!-- ============================================================== -->
                    ...
                    <div class="card-body">
                        ...
                        <div class="col-12 mb-4">
                            <h5 class="card-title text-center">Kesehatan Bayi Baru Lahir</h5>  
                        </div> 
                            ...
                            <div class="form-group col-4" style="margin-top:-25px;">
                                <label for="agama_ibu">Pilih Data</label>
                                    <select class="form-select @error('agama_ibu') is-invalid @enderror"  aria-label="Default select example" name="agama_ibu" id="agama_ibu" onchange="pindahHalaman(this.value)">
                                          ...
                                          <option value= "/tinggibadan" {{ old('agama_ibu') }}>Tinggi Badan Berdasarkan Umur</option>
                                    </select>
                                          @error('agama_ibu')
                                          <div class="invalid-feedback">
                                            {{ $message }}
                        ...
                        <div class="col-4">
                            <button type="button" class="btn btn-outline-dark rounded-pill fw-bold text-white" style="background-color: #4DA8DA;margin-left: 250px;" onclick="window.location='{{ url('/tambahkesehatanbayibarulahir') }}';" style="margin-left: 75px; background-color: #EE66A6;">Tambah</button>
                        </div>
                        </div>
                       ...
                       <div class="card-body">
                        <div class="table-responsive bg-white">
                            <table class="table table-bordered table-hover text-center">
                                ...
                                <tbody>
                                    @php
                                    $nomor = 1 + (( $datakesehatanbayibarulahir->currentPage()-1)* $datakesehatanbayibarulahir->perPage());;
                                @endphp
                                    @foreach ($datakesehatanbayibarulahir as $row)
                                    <tr>
                                        <td>{{ $nomor ++ }}</td>
                                        <td>{{ $row->nama_anak }}</td>
                                        <td>{{ $row->nama_ibu }}</td>
                                        ...
                                        <td>  <div class="row">
                                            <div class="col-6 text-center">
                                                <button onclick="window.location='{{ url('kesehatanbayibarulahir/'.$row->id) }}'" type="button" class="btn btn-success btn-md" title="Detail">
                                                     <i class="fas fa-list"></i>
                                                </button>
                                               </div>
                                            <div class="col-6 text-center">
                                             <form onsubmit="return deleteData('{{ $row->nama_anak }}')" action="{{ url('kesehatanbayibarulahir/'.$row->id) }}" method="POST"> 
                                              @csrf
                                              @method('DELETE')
                            ...
                            {!! $datakesehatanbayibarulahir->appends(Request::except('page'))->render() !!}
                        </div>
                    </div>
                      <script>
