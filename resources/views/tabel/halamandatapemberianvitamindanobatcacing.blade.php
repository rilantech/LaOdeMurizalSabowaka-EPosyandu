                ...
                <!-- ============================================================== -->
                    ...
                    <div class="card-body">
                        ...
                        <div class="col-12 mb-4">
                            <h5 class="card-title text-center">Data Pemberian Vitamin & Obat Cacing</h5>  
                        </div> 
                        ...
                        <div class="col-4">
                            <button type="button" class="btn btn-outline-dark rounded-pill fw-bold text-white" style="background-color: #4DA8DA;margin-left: 250px;" onclick="window.location='{{ url('/tambahpemberianvitamindanobatcacing') }}';" style="margin-left: 75px; background-color: #EE66A6;">Tambah</button>
                        </div>
                        </div>
                       ...
                       <div class="card-body">
                        <div class="table-responsive bg-white">
                            <table class="table table-bordered table-hover text-center">
                                ...
                                <tbody>
                                    @php
                                    $nomor = 1 + (( $datapemberianvitamindanobatcacing->currentPage()-1)* $datapemberianvitamindanobatcacing->perPage());;
                                @endphp
                                    @foreach ($datapemberianvitamindanobatcacing as $row)
                                    <tr>
                                        <td>{{ $nomor ++ }}</td>
                                        <td>{{ $row->nama_anak }}</td>
                                        <td>{{ $row->nama_ibu }}</td>
                                        ...
                                        <td>  <div class="row">
                                            <div class="col-6 text-center">
                                                <button onclick="window.location='{{ url('pemberianvitamindanobatcacing/'.$row->id) }}'" type="button" class="btn btn-success btn-md" title="Detail">
                                                     <i class="fas fa-list"></i>
                                                </button>
                                               </div>
                                            <div class="col-6 text-center">
                                             <form onsubmit="return deleteData('{{ $row->nama_anak }}')" action="{{ url('pemberianvitamindanobatcacing/'.$row->id) }}" method="POST"> 
                                              @csrf
                                              @method('DELETE')
                            ...
                            {!! $datapemberianvitamindanobatcacing->appends(Request::except('page'))->render() !!}
                        </div>
                    </div>
                    <script>
