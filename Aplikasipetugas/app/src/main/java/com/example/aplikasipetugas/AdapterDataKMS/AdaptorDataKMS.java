...
import com.example.aplikasipetugas.Data_KMS;
...
public class AdaptorDataKMS extends BaseAdapter {
    ...
    public View getView(final int position, View view, ViewGroup viewGroup) {
        ...
        view1 = this.inflater.inflate((int) R.layout.list_data_kms, (ViewGroup) null);
        if (view1 != null) {
            ...
            ((ImageView) view1.findViewById(R.id.tomboleksport)).setOnClickListener(new View.OnClickListener() {
                public void onClick(View view) {
                    Intent intent = new Intent(AdaptorDataKMS.this.context.getApplicationContext(), Data_KMS.class);
                    intent.putExtra("UploadTaskParameters.Companion.CodingKeys.id", AdaptorDataKMS.this.model.get(position).getId());
                    intent.setFlags(Intent.FLAG_ACTIVITY_REORDER_TO_FRONT|Intent.FLAG_ACTIVITY_NEW_TASK);
                    AdaptorDataKMS.this.context.getApplicationContext().startActivity(intent);
                }
            });
