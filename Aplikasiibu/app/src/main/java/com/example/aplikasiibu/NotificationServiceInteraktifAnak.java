...
public class NotificationServiceInteraktifAnak extends Service {
    ...
    private Runnable runnable = new Runnable() {
        public void run() {
            if (HalamanLogin.status != null) {
                NotificationServiceInteraktifAnak.this.handler.postDelayed(this, 1000);
                NotificationServiceInteraktifAnak.this.tampilkanNotifikasiInteraktif();
            }
        }
    };
        ...
        Volley.newRequestQueue(this).add(new StringRequest(0, new Configurasi().baseUrl() + "api/notifinteraktifanak", new Response.Listener<String>() {
            public void onResponse(String response) {
                try {
                    ...
                    NotificationServiceInteraktifAnak.nama_anak = jsonObject.getString("nama_anak");
                    if (status.equals("notif_nol_tiga")) {
                        NotificationServiceInteraktifAnak.this.notif_nol_tiga();
                    }
                    if (status.equals("notif_tiga_enam")) {
                        NotificationServiceInteraktifAnak.this.notif_tiga_enam();
                    }
                    if (status.equals("notif_enam_sembilan")) {
                        NotificationServiceInteraktifAnak.this.notif_enam_duabelas();
                    }
                    if (status.equals("notif_satu_duatahun")) {
                        NotificationServiceInteraktifAnak.this.notif_satu_duatahun();
                    }
                    if (status.equals("notif_dua_tigatahun")) {
                        NotificationServiceInteraktifAnak.this.notif_dua_tigatahun();
                    }
                    if (status.equals("notif_tiga_limatahun")) {
                        NotificationServiceInteraktifAnak.this.notif_tiga_limatahun();
                    }
